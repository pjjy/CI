...
class CA_PDF extends FPDF
{
	...
	function ca_Slip_form($rec,$no,$data,$date)
	{
		...
		$this->Cell(2.5, 5,'', 0, 0, "L");
		$this->Cell(25.5, 5,'PREPARED BY :        VILLARIN, ANA MARIA CORE', '', 0, "L");
...
foreach ($report_unaudited_all as  $value){
  $bu_array = array();
 
    // public function getdept_unaudited_bypcc_mod($locbu,$date,$pcc_code){
   $data = $this->Accounting_mod->getdept_unaudited_all_mod($value['bunit_code'],$value['company_code']);
