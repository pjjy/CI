...
class Accounting_mod extends CI_Model {
                ...
                <tbody>';
			        ...
			        if(!empty($ded_Data)){
		                  ...
		                  echo '<tr style="font-size:13px">';
		                    echo '<td>'.$value['ded_datededucted'].'</td>';
			        ...
			        if(!empty($emp_Data))
			        {
			        	...
			        	else
			        	{
				        	...
				        	$sched 	= $this->get_cutoff($emp_Data['company_code'],$emp_Data['bunit_code']);
				        	var_dump($div);
			        	}	
		            }
		             echo 
    ...
    public function get_cutoff($cc,$bc,$date)
    {
    	...
    	foreach ($data as $value):
	    	...
	    	$cutoff[] = $value['pDaySC'];
	    		...
	    		elseif(date('d',strtotime($date)) == '30')
	    		{
	    			$cutoff[] = '30';
	    		}
	    	endif;
    ...
    public function hb_getdept_unaudited_all_mod($locbu,$date){
    ...
    public function sub_walkinpay_mod($p_wlkin_acc_no,$p_date_v,$p_payamt_v,$p_ornumber_v){
    	...
    	$data = array(
	       ...
	       'ded_amount'			=> str_replace(',','', $p_payamt_v),
	       ...
	       'ded_payable'        => $emp_Data[0]['mhhp_current_bal']-str_replace(',','', $p_payamt_v) 
		);
		$this->db->insert('ebs.mhhp_deduction', $data);

		$data1 = array(
			'mhhp_current_bal' => $emp_Data[0]['mhhp_current_bal']-str_replace(',','', $p_payamt_v)
		);
